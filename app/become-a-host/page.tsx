      ...
      {file ? (
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">
            Drop your ID here or <span className="text-amber-400">browse</span>
          </p>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-white px-6 text-center">
      ...
      <p className="mb-8 max-w-sm text-sm text-muted leading-relaxed">
        You'll receive an email at your registered address once your host account is approved.
      </p>
      <button
        onClick={() => router.push("/")}
...
export default function BecomeHostPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${config.apiUrl}/api/host-applications/`, {
        method: "POST",
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-xs text-muted">Please sign in to apply as a host.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-xs text-muted">Head to your dashboard to manage your properties.</p>
          <button
            onClick={() => router.push("/")}
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-10">
          ...
          <p className="text-sm text-muted leading-relaxed">
            List your property on Rent Apart. We verify every host to keep tenants safe —
            this takes 1–3 business days.
          </p>
          {userEmail && (
            <p className="mt-2 text-xs text-muted">
              Applying as <span className="text-foreground">{userEmail}</span>
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-8">
          ...
          <section className="rounded-2xl border border-border bg-card/40 p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {field("City / Municipality", "city", { placeholder: "Cebu City" })}
          ...
          <section className="rounded-2xl border border-border bg-card/40 p-6">
            ...
            <p className="mb-5 text-xs text-muted">
              Required under BSP and DTI guidelines for property lease agreements in the Philippines.
            </p>
            <div className="grid gap-4 sm:grid-cols-2">
              {select("ID Type", "idType", ID_TYPES)}
              {field("ID Number", "idNumber", { placeholder: "e.g. 1234-5678-9012" })}
              <div className="sm:col-span-2">
                ...
                />
                {errors.idFile && (
                  <p className="mt-1 text-xs text-red-400">{errors.idFile}</p>
                )}
              </div>
            </div>
          </section>

          {/* Section: Property Intent */}
          <section className="rounded-2xl border border-border bg-card/40 p-6">
            <h2 className="mb-5 text-xs font-bold uppercase tracking-widest text-amber-400">
              04 · Property Intent
            </h2>
            <div className="grid gap-4 sm:grid-cols-2">
              {select("Type of Property", "propertyType", PROPERTY_TYPES)}
          ...
          <div className="rounded-2xl border border-border bg-card/40 p-6">
            <label className="flex cursor-pointer items-start gap-3">
              ...
              <span className="text-sm text-muted leading-relaxed">
                I confirm that all information provided is accurate and true. I understand
                that submitting false documents is a violation of Philippine law (RA 11032,
                Civil Code Art. 19–21) and may result in account termination.
              </span>
            </label>
            {errors.agreedToTerms && (
              <p className="mt-2 text-xs text-red-400">{errors.agreedToTerms}</p>
            )}
          </div>
